"""GPT-2-small LM rankpath body window [4:16) on Distil 12, locked before LRs."""
...
PROTOCOL = ROOT / "research" / "PROTOCOL-isolated-rankpath-g2dbody.md"
LOG = ROOT / "research" / "LOGBOOK.md"
PAIR = ROOT / "experiments" / "2026-08-31-pair-distilgpt2-12x4"
GPT2_OPEN = ROOT / "experiments" / "2026-09-04-probe-distil-12x4-rankpath-gpt2-lm"
DISTIL_NATIVE_BODY = (
    ROOT
    / "experiments"
...
def test_protocol_rankpath_g2dbody_locks_config_before_lrs() -> None:
    text = PROTOCOL.read_text()
    assert "25/48" in text
    assert "H-rpg2dbody" in text
    assert "H-rpg2dbody-iso" in text
    assert "H-rpg2dbody **fails**" in text
    assert "H-rpg2dbody-iso **holds**" in text
    ...
    assert "2026-09-04-probe-distil-12x4-rankpath-gpt2-w4-16" in text
    assert "Do **not** look at GPT-2-on-Distil rankpath `[4:16)` LRs" in text
    ...
    log = LOG.read_text()
    assert "PROTOCOL-isolated-rankpath-g2dbody" in log
    assert "named before" in log
    assert "PROTOCOL-isolated-rankpath-g2dbody" in (
        ROOT / "research" / "results-ledger.md"
    ).read_text()
    assert "PROTOCOL-isolated-rankpath-g2dbody" in (ROOT / "HOW-TO.md").read_text()
    exp = [
        ln
        for ln in (ROOT / "experiments" / "README.md").read_text().splitlines()
        if "2026-09-04-probe-distil-12x4-rankpath-gpt2-w4-16" in ln
    ]
    assert len(exp) == 1
    research = [
        ln
        for ln in (ROOT / "research" / "README.md").read_text().splitlines()
        if "PROTOCOL-isolated-rankpath-g2dbody" in ln
    ]
    assert len(research) == 1
    agents = [
        ln
        for ln in (ROOT / "AGENTS.md").read_text().splitlines()
        if ln.startswith("| GPT-2-on-Distil rankpath [4:16)")
    ]
    assert len(agents) == 1
    assert "**25/48**" in agents[0]
    assert "**26/48 vs 21/48**" in agents[0]
    lo, hi = clopper_pearson(25, 48)
    assert lo <= 0.5 <= hi


def test_protocol_rankpath_g2dbody_cli_flag_exists() -> None:
    ...
    args = build_parser().parse_args(
        [
            ...
            "4:16",
            "--out-dir",
            "experiments/2026-09-04-probe-distil-12x4-rankpath-gpt2-w4-16",
        ]
    )
...
def test_protocol_rankpath_g2dbody_opening_control_stays() -> None:
    holdout = json.loads((GPT2_OPEN / "rankpath" / "holdout.json").read_text())
...
def test_protocol_rankpath_g2dbody_distil_native_body_control_stays() -> None:
    holdout = json.loads((DISTIL_NATIVE_BODY / "rankpath" / "holdout.json").read_text())
    assert holdout["used_keys"] is False
...
def test_protocol_rankpath_g2dbody_from_dumps() -> None:
    ...
    assert interp["n_marked_lr_positive"] == 26
    assert interp["n_unmarked_lr_nonpositive"] == 21
    text = PROTOCOL.read_text()
    assert "H-rpg2dbody **fails**" in text
