"""gpt2-medium-LM rankpath body window [4:16), locked before LRs."""
...
PROTOCOL = ROOT / "research" / "PROTOCOL-isolated-rankpath-mbody.md"
LOG = ROOT / "research" / "LOGBOOK.md"
PAIR = ROOT / "experiments" / "2026-08-17-pair-12x4"
MEDIUM_OPEN = ROOT / "experiments" / "2026-09-04-probe-12x4-rankpath-medium-lm"
GPT2_BODY = (
    ROOT
    / "experiments"
...
PROBE = (
    ROOT
    / "experiments"
    / "2026-09-04-probe-12x4-rankpath-medium-w4-16"
    / "window-4-16"
)


def test_protocol_rankpath_mbody_locks_config_before_lrs() -> None:
    ...
    assert "2026-09-04-probe-12x4-rankpath-medium-w4-16" in text
    assert "Do **not** look at gpt2-medium-LM rankpath `[4:16)` LRs" in text
    ...
    log = LOG.read_text()
    assert "PROTOCOL-isolated-rankpath-mbody" in log
    assert "named before" in log
    assert "PROTOCOL-isolated-rankpath-mbody" in (ROOT / "research" / "results-ledger.md").read_text()
    assert "PROTOCOL-isolated-rankpath-mbody" in (ROOT / "HOW-TO.md").read_text()
    exp = [
        ln
        for ln in (ROOT / "experiments" / "README.md").read_text().splitlines()
        if "2026-09-04-probe-12x4-rankpath-medium-w4-16" in ln
    ]
    assert len(exp) == 1
    research = [
        ln
        for ln in (ROOT / "research" / "README.md").read_text().splitlines()
        if "PROTOCOL-isolated-rankpath-mbody" in ln
    ]
    assert len(research) == 1
    agents = [
        ln
        for ln in (ROOT / "AGENTS.md").read_text().splitlines()
        if ln.startswith("| medium-LM rankpath [4:16)")
    ]
    assert len(agents) == 1
...
def test_protocol_rankpath_mbody_cli_flag_exists() -> None:
    ...
    args = build_parser().parse_args(
        [
            ...
            "4:16",
            "--out-dir",
            "experiments/2026-09-04-probe-12x4-rankpath-medium-w4-16",
        ]
    )
...
def test_protocol_rankpath_mbody_medium_opening_control_stays() -> None:
    ...
    assert holdout["n_marked_lr_positive"] == 31
    assert holdout["n_unmarked_lr_nonpositive"] == 32


def test_protocol_rankpath_mbody_gpt2_body_control_stays() -> None:
...
def test_protocol_rankpath_mbody_distil_body_control_stays() -> None:
