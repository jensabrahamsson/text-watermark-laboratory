"""Distil native rankpath body window [4:16), locked before LRs."""
...
PROTOCOL = ROOT / "research" / "PROTOCOL-isolated-rankpath-d12body.md"
LOG = ROOT / "research" / "LOGBOOK.md"
PAIR = ROOT / "experiments" / "2026-08-31-pair-distilgpt2-12x4"
DISTIL_OPEN = ROOT / "experiments" / "2026-08-31-probe-distilgpt2-12x4-fitprefix4-rankpath"
DISTIL_ON_GPT2 = (
    ROOT
    / "experiments"
...
def test_protocol_rankpath_d12body_locks_config_before_lrs() -> None:
    text = PROTOCOL.read_text()
    assert "25/48" in text
    assert "H-rpd12body" in text
    assert "H-rpd12body-iso" in text
    assert "H-rpd12body **holds**" in text
    assert "H-rpd12body-iso **holds**" in text
    ...
    assert "2026-09-04-probe-distil-12x4-rankpath-w4-16" in text
    assert "Do **not** look at Distil native rankpath `[4:16)` LRs" in text
    ...
    log = LOG.read_text()
    assert "PROTOCOL-isolated-rankpath-d12body" in log
    assert "named before" in log
    assert "PROTOCOL-isolated-rankpath-d12body" in (
        ROOT / "research" / "results-ledger.md"
    ).read_text()
    assert "PROTOCOL-isolated-rankpath-d12body" in (ROOT / "HOW-TO.md").read_text()
    exp = [
        ln
        for ln in (ROOT / "experiments" / "README.md").read_text().splitlines()
        if "2026-09-04-probe-distil-12x4-rankpath-w4-16" in ln
    ]
    assert len(exp) == 1
    research = [
        ln
        for ln in (ROOT / "research" / "README.md").read_text().splitlines()
        if "PROTOCOL-isolated-rankpath-d12body" in ln
    ]
    assert len(research) == 1
    agents = [
        ln
        for ln in (ROOT / "AGENTS.md").read_text().splitlines()
        if ln.startswith("| Distil native rankpath [4:16)")
    ]
    assert len(agents) == 1
    assert "**25/48**" in agents[0]
    assert "**25/48 vs 30/48**" in agents[0]
    lo, hi = clopper_pearson(25, 48)
    assert lo <= 0.5 <= hi


def test_protocol_rankpath_d12body_cli_flag_exists() -> None:
    ...
    args = build_parser().parse_args(
        [
            ...
            "4:16",
            "--out-dir",
            "experiments/2026-09-04-probe-distil-12x4-rankpath-w4-16",
        ]
    )
...
def test_protocol_rankpath_d12body_native_opening_control_stays() -> None:
...
def test_protocol_rankpath_d12body_distil_on_gpt2_body_control_stays() -> None:
    holdout = json.loads((DISTIL_ON_GPT2 / "rankpath" / "holdout.json").read_text())
    assert holdout["used_keys"] is False
    assert holdout["model_name"] == "distilgpt2"
...
def test_protocol_rankpath_d12body_from_dumps() -> None:
