...
def test_protocol_rankpath_m2d_locks_config_before_lrs() -> None:
    ...
    assert "H-rpm2d **fails**" in text
    assert "H-rpm2d-iso **holds**" in text
    ...
    research = [
        ...
        if "PROTOCOL-isolated-rankpath-m2d.md" in ln
        and "m2dbody" not in ln
    ]
    ...
    assert "**30/48 vs 33/48**" in agents[0]
