"""Distil LM rankpath body window [4:16) on gpt2-medium 12, locked before LRs."""
...
PROTOCOL = ROOT / "research" / "PROTOCOL-isolated-rankpath-d2mbody.md"
LOG = ROOT / "research" / "LOGBOOK.md"
PAIR = ROOT / "experiments" / "2026-09-01-pair-gpt2-medium-12x4"
OPENING = ROOT / "experiments" / "2026-09-04-probe-medium-12x4-rankpath-distil-lm"
NATIVE_BODY = (
    ROOT
    / "experiments"
...
def test_protocol_rankpath_d2mbody_locks_config_before_lrs() -> None:
    text = PROTOCOL.read_text()
    assert "25/48" in text
    assert "H-rpd2mbody" in text
    assert "H-rpd2mbody-iso" in text
    assert "H-rpd2mbody **holds**" in text
    assert "H-rpd2mbody-iso **holds**" in text
    assert "Do not sell **25/48**" in text
    ...
    assert "2026-09-04-probe-medium-12x4-rankpath-distil-w4-16" in text
    assert "Do **not** look at Distil-on-medium rankpath `[4:16)` LRs" in text
    ...
    log = LOG.read_text()
    assert "PROTOCOL-isolated-rankpath-d2mbody" in log
    assert "named before" in log
    assert "PROTOCOL-isolated-rankpath-d2mbody" in (
        ROOT / "research" / "results-ledger.md"
    ).read_text()
    assert "PROTOCOL-isolated-rankpath-d2mbody" in (ROOT / "HOW-TO.md").read_text()
    exp = [
        ln
        for ln in (ROOT / "experiments" / "README.md").read_text().splitlines()
        if "2026-09-04-probe-medium-12x4-rankpath-distil-w4-16" in ln
    ]
    assert len(exp) == 1
    research = [
        ln
        for ln in (ROOT / "research" / "README.md").read_text().splitlines()
        if "PROTOCOL-isolated-rankpath-d2mbody.md" in ln
    ]
    assert len(research) == 1
    agents = [
        ln
        for ln in (ROOT / "AGENTS.md").read_text().splitlines()
        if ln.startswith("| Distil-on-medium rankpath [4:16)")
    ]
    assert len(agents) == 1
    assert "**25/48**" in agents[0]
    assert "**25/48 vs 33/48**" in agents[0]
    lo, hi = clopper_pearson(25, 48)
    assert lo <= 0.5 <= hi


def test_protocol_rankpath_d2mbody_cli_flag_exists() -> None:
    ...
    args = build_parser().parse_args(
        [
            ...
            "4:16",
            "--out-dir",
            "experiments/2026-09-04-probe-medium-12x4-rankpath-distil-w4-16",
        ]
    )
...
def test_protocol_rankpath_d2mbody_opening_control_stays() -> None:
...
def test_protocol_rankpath_d2mbody_native_body_control_stays() -> None:
...
def test_protocol_rankpath_d2mbody_from_dumps() -> None:
    ...
    assert interp["n_marked_lr_positive"] == 25
