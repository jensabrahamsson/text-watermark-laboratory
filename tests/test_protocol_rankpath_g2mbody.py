"""GPT-2-small LM rankpath body window [4:16) on gpt2-medium 12, locked before LRs."""
...
PROTOCOL = ROOT / "research" / "PROTOCOL-isolated-rankpath-g2mbody.md"
LOG = ROOT / "research" / "LOGBOOK.md"
PAIR = ROOT / "experiments" / "2026-09-01-pair-gpt2-medium-12x4"
OPENING = ROOT / "experiments" / "2026-09-04-probe-medium-12x4-rankpath-gpt2-lm"
DISTIL_BODY = (
    ROOT
    / "experiments"
    / "2026-09-04-probe-medium-12x4-rankpath-distil-w4-16"
    / "window-4-16"
)
NATIVE_BODY = (
    ROOT
    / "experiments"
    / "2026-09-04-probe-medium-12x4-rankpath-w4-16"
    / "window-4-16"
)
PROBE = (
    ROOT
    / "experiments"
    / "2026-09-04-probe-medium-12x4-rankpath-gpt2-w4-16"
    / "window-4-16"
)


def test_protocol_rankpath_g2mbody_locks_config_before_lrs() -> None:
    text = PROTOCOL.read_text()
    assert "25/48" in text
    assert "H-rpg2mbody" in text
    assert "H-rpg2mbody-iso" in text
    assert "H-rpg2mbody **fails**" in text
    assert "H-rpg2mbody-iso **holds**" in text
    assert "Do not sell **28/48**" in text
    ...
    assert "2026-09-04-probe-medium-12x4-rankpath-gpt2-w4-16" in text
    assert "Do **not** look at GPT-2-on-medium rankpath `[4:16)` LRs" in text
    ...
    log = LOG.read_text()
    assert "PROTOCOL-isolated-rankpath-g2mbody" in log
    assert "named before" in log
    assert "PROTOCOL-isolated-rankpath-g2mbody" in (
        ROOT / "research" / "results-ledger.md"
    ).read_text()
    assert "PROTOCOL-isolated-rankpath-g2mbody" in (ROOT / "HOW-TO.md").read_text()
    exp = [
        ln
        for ln in (ROOT / "experiments" / "README.md").read_text().splitlines()
        if "2026-09-04-probe-medium-12x4-rankpath-gpt2-w4-16" in ln
    ]
    assert len(exp) == 1
    research = [
        ln
        for ln in (ROOT / "research" / "README.md").read_text().splitlines()
        if "PROTOCOL-isolated-rankpath-g2mbody.md" in ln
    ]
    assert len(research) == 1
    agents = [
        ln
        for ln in (ROOT / "AGENTS.md").read_text().splitlines()
        if ln.startswith("| GPT-2-on-medium rankpath [4:16)")
    ]
    assert len(agents) == 1
    assert "**25/48**" in agents[0]
    assert "**28/48 vs 30/48**" in agents[0]
    lo, hi = clopper_pearson(25, 48)
    assert lo <= 0.5 <= hi


def test_protocol_rankpath_g2mbody_cli_flag_exists() -> None:
    ...
    args = build_parser().parse_args(
        [
            ...
            "4:16",
            "--out-dir",
            "experiments/2026-09-04-probe-medium-12x4-rankpath-gpt2-w4-16",
        ]
    )
...
def test_protocol_rankpath_g2mbody_opening_control_stays() -> None:
...
def test_protocol_rankpath_g2mbody_distil_body_control_stays() -> None:
    holdout = json.loads((DISTIL_BODY / "rankpath" / "holdout.json").read_text())
    assert holdout["used_keys"] is False
...
def test_protocol_rankpath_g2mbody_native_body_control_stays() -> None:
...
def test_protocol_rankpath_g2mbody_from_dumps() -> None:
    ...
    assert interp["n_marked_lr_positive"] == 28
