...
| In-domain hashtok `--n-hashes` | Mixer-width ablation of that reader | n=2 **11/12** / **34/48 vs 31/48** nested **28/48 vs 37/48**; n=4 **36/48 vs 30/48**; n=32 hurts **30/48**; default n=8 is not the best in-domain width; do not sell 36/48 |
...
This lab did **not** invent key-free watermark detection. Canonical map: [research/related-work.md](research/related-work.md). Citation convention: [research/CITING.md](research/CITING.md). Annotated records: [research/annotated-bibliography.md](research/annotated-bibliography.md). BibTeX: [research/references.bib](research/references.bib).

Wang et al. (2026) (*TTP-Detect*) formulate third-party, key-agnostic verification from observable outputs and paired watermarked/unwatermarked reference sets — the same audit problem, a different method. Gloaguen et al. (2025) detect a watermarking scheme as a *generator property* with limited black-box queries ([ETH/SRI write-up](https://www.sri.inf.ethz.ch/blog/probingsynthid)). That is not scoring a finished string against count tables. Omidi et al. (2026) analyse SynthID-Text’s **keyed** mean and Bayesian scores. That is official g-value detection, not `indicate`. Duan et al. (2025) (*PVMark*) prove keyed detection in zero knowledge; the detector still uses the key. Jovanović et al. (2024) is watermark stealing (arXiv 2402.19361), not Zhang et al. (2024) (arXiv 2311.04378).
