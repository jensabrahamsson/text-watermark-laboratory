...
| 3 | Isolated hard sign, one marked file, `lr>0` | **25/48**; Clopper–Pearson 95% **[0.372, 0.667]** includes ½ | A 5% binomial test; not a calibrated detector |
...
**Mechanistic finding.** Scoring the first four generated tokens already ranks **34/36** in-domain, matching a 16-token prefix: an opening is **sufficient** on that corpus. Truncated last-k contexts used to be stored `context_len` times; that overcount inflated the published **10/12** / **29/48**. After storing each real suffix once, prompt ranking is **9/12** (or **10/12** with margin 0.02) and isolated hard sign is **25/48**. In-domain 36×4 hits is still **36/36**. PROTOCOL-next H2 reindexed windows were **0:4** **99/100** vs **16:32** **89/100**; absolute-history remasure is **0:4** **99/100** vs **16:32** **87/100** ([research/PROTOCOL-h2-absolute.md](research/PROTOCOL-h2-absolute.md)). Paired prompt-family McNemar on those absolute windows is **13 vs 1** among 14 discordant families; unpaired Clopper–Pearson overlap is not that test. File-level permutation p = 0.040 on the original 12 is descriptive; the inferential unit is the prompt family. Occupancy-free isolated recall on named opening observed-token readers is bounded by opening-atom overlap.
...
**Single-text classification (25/48).** Same leave-one-out tables, sign of one file’s LR against 0, no twin. Hard last-4 marked `lr>0`: **25/48**. Unmarked `lr≤0`: **22/48**. Binomial P(≥25 | n=48, p=0.5) ≈ 0.44. Clopper–Pearson 95% **[0.372, 0.667]** includes ½. Ranking of the same LRs is AUC **0.590** (file-level permutation p = 0.040, descriptive). Pre-fix **29/48** / AUC 0.626 overcounted truncated openings. That overlap is why 9/12 must not be read as per-file accuracy. Hard last-4 **9/12** has Clopper–Pearson **[0.428, 0.945]** (includes ½); the margin-0.02 **10/12** interval excludes ½ and is a robustness check, not the headline. The two grains also disagree on stems: garden is one of the nine ranking wins with **0/4** marked files above 0; station, office, and ferry-queue *lose* ranking but hold **5** of the **25** isolated TPs. `blind` leave-one-prompt-out now prints that split. Table: `experiments/2026-09-01-ranking-isolated-honesty/`.
