...
`--snaprate` is the matching *detection* baseline: the same unmarked-LM rows, no twin tables, no keys. `snapleave` / `snapupset` / `snapmiss` report whether leaving the argmax (or missing top-k) already classifies an isolated opening. Rank-path tables ask *which* rank was chosen; snap-rate only asks *whether* it was the argmax.
