...
- Opening-only unmarked-LM geometry (generated tokens 1–3) beats the published 128-token pivot in-domain (**27/48** vs **17/48**) but **does not transfer** (OOD LDA 15/48, AUC 0.422). Coverage-then-pivot cascade is an honest two-channel report: in-domain 36/48 with unmarked FPs; combined 60-stem train covers **42/48** of which **34/48** have `lr>0` (precision 1.0 among decided); cascade 39/48 spends 15 unmarked FPs. Do not sell cascade 39/48 as beating poshits 39/48. `indicate score` prints **ABSTAIN** at `n_used=0`.
