# Isolated-file protocol (GPT-2-small LM rankpath body window [4:16) on gpt2-medium 12)

This is a **methods freeze** for 12-LOO rankpath on already frozen
gpt2-medium 12×4 twins, with **GPT-2-small** as the unmarked LM, scoring
...
twins is **8/12**, isolated **20/48 vs 32/48**
([PROTOCOL-isolated-rankpath-g2m.md](PROTOCOL-isolated-rankpath-g2m.md)).
Distil-on-medium `[4:16)` is **11/12**, isolated **25/48 vs 33/48**
...
This freeze asks whether GPT-2-small ranks on gpt2-medium twins still
...
file.
Do **not** look at GPT-2-on-medium rankpath `[4:16)` LRs until this
...
GPT-2-small LM opening on gpt2-medium 12 is ranking **8/12**, isolated
**20/48 vs 32/48**. Distil-on-medium `[4:16)` equality with **25/48** is
not a win. Isolated-file remains open. Occupancy leftover is closed.
Existing medium twins. Existing `rankpath`. Existing `--windows`. Not a
leftover re-slice.

## Primary scientific question

Under leave-one-prompt-out GPT-2-small-LM rankpath on `[4:16)`, with no
detector keys, `hash_iv`, or g-values, do GPT-2 ranks classify
...
- **H-rpg2mbody.** GPT-2-small unmarked-LM rankpath on gpt2-medium
  generated tokens `[4:16)` isolated marked `lr>0` does not beat hard
  **25/48**. Opening on these twins is **20/48 vs 32/48**. Distil-on-medium
  `[4:16)` is **25/48 vs 33/48**. A higher count is not leftover-15
  recall and does not replace **25/48**. Equality with **25/48** is not
  a win.
- **H-rpg2mbody-iso.** A GPT-2-on-medium body-window rankpath is not a
  universal isolated-file detector. Do not sell that isolated count,
  GPT-2-on-medium opening **20/48**, Distil-on-medium `[4:16)` **25/48**,
...
| GPT-2-on-medium opening `[0:4)` control | `experiments/2026-09-04-probe-medium-12x4-rankpath-gpt2-lm/` |
| Distil-on-medium `[4:16)` control | `experiments/2026-09-04-probe-medium-12x4-rankpath-distil-w4-16/window-4-16/` |
| Probe dump | `experiments/2026-09-04-probe-medium-12x4-rankpath-gpt2-w4-16/` |
...
python -m text_watermark_tools probe experiments/2026-09-01-pair-gpt2-medium-12x4 \
  ...
  --windows 4:16 \
  --out-dir experiments/2026-09-04-probe-medium-12x4-rankpath-gpt2-w4-16
```
...
H-rpg2mbody **fails** as a raw count. GPT-2-small unmarked-LM rankpath on
gpt2-medium generated tokens `[4:16)` ranks **8/12**; isolated **28/48 vs 30/48**
(AUC **0.624**; nested-by-stem Youden **25/48 vs 28/48**).
`used_keys=false`. `model_name` is `gpt2`. Clopper–Pearson 95% on
**28/48** includes ½. GPT-2-on-medium opening `[0:4)` stays **8/12**,
isolated **20/48 vs 32/48**. Distil-on-medium `[4:16)` stays **11/12**,
isolated **25/48 vs 33/48**. The unwindowed fit-prefix-16 file score on
this run is **8/12**, isolated **25/48 vs 31/48**; that is not the frozen
slice and is not the locked headline **25/48**. Do not sell **28/48**,
**8/12**, or parent **25/48**. JSON:
[experiments/2026-09-04-probe-medium-12x4-rankpath-gpt2-w4-16/window-4-16/](../experiments/2026-09-04-probe-medium-12x4-rankpath-gpt2-w4-16/window-4-16/).

H-rpg2mbody-iso **holds**. Isolated **28/48** is three files above hard
