# Isolated-file protocol (gpt2-medium LM on DistilGPT2 12×4)

This is a **methods freeze** for opening rankpath 12-LOO on already
frozen DistilGPT2 12×4 twins, with **gpt2-medium** as the unmarked LM.
...
**28/48 vs 32/48**. GPT-2-small LM on these twins is **6/12**, isolated
**24/48 vs 27/48**
([PROTOCOL-isolated-rankpath-g2d.md](PROTOCOL-isolated-rankpath-g2d.md)).
gpt2-medium native on gpt2-medium twins is **6/12**, isolated
**22/48 vs 30/48**. Distil LM on gpt2-medium twins is **11/12**, isolated
**30/48 vs 31/48**. This freeze asks whether gpt2-medium ranks still
fire on Distil generations of the same 12 Grok seeds.

Author–year citations follow [CITING.md](CITING.md). Locked headlines
remain **9/12**, **25/48**, and **36/36**. Pre-fix **10/12** /
**29/48** stay in historical JSON. Do **not** write `thesis/` from this
file.
Do **not** look at gpt2-medium-on-Distil rankpath LRs until this freeze
...
gpt2-medium LM scoring Distil generations. Distil native is chance
**8/12**. GPT-2-small on Distil is chance file-AUC **0.532**. Isolated-file
detection is still not finished. Existing twins. Existing `rankpath`.
Not a leftover re-slice.

## Primary scientific question

Under leave-one-prompt-out opening rankpath, with no detector keys,
`hash_iv`, or g-values, do gpt2-medium ranks classify DistilGPT2 12×4
...
|---|---|---|
| DistilGPT2 12×4 | gpt2-medium | `gpt2-medium` |
...
- **H-rpm2d.** gpt2-medium unmarked-LM opening rankpath on DistilGPT2
  12×4 isolated marked `lr>0` does not beat hard **25/48**. Distil
  native is **28/48 vs 32/48**. GPT-2-small on Distil is
  **24/48 vs 27/48**. A higher count is not leftover-15 recall and does
  not replace **25/48**.
- **H-rpm2d-iso.** Cross-size opening rankpath is not a universal
  isolated-file detector. Do not sell that isolated count, Distil
  native **28/48**, GPT-2-on-Distil **24/48**, GPT-2-on-medium
  **20/48**, medium native **22/48**, Distil-LM-on-GPT-2 **32/48**,
  Distil-on-medium **30/48**, GPT-2-small **41/48**, leftover official
...
`model_name` must be `gpt2-medium`. Equality with **25/48** is not a win.
...
| GPT-2-small on Distil control | `experiments/2026-09-04-probe-distil-12x4-rankpath-gpt2-lm/` |
| Probe dump | `experiments/2026-09-04-probe-distil-12x4-rankpath-medium-lm/` |
...
python -m text_watermark_tools probe experiments/2026-08-31-pair-distilgpt2-12x4 \
  ...
  --out-dir experiments/2026-09-04-probe-distil-12x4-rankpath-medium-lm
```
