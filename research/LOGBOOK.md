...
is still not finished. Do not write `thesis/`. Freeze SHA `68b0514`.

## 2026-09-04 Distil-LM rankpath body [4:16) opened

H-rpdbody **holds**. Distil-LM rankpath on `[4:16)` is ranking **6/12**,
isolated **24/48 vs 23/48** (AUC **0.447**; nested **32/48 vs 19/48** at
a negative threshold). `used_keys=false`. Distil-LM opening stays
**10/12**, isolated **32/48 vs 31/48**. GPT-2 `[4:16)` stays **7/12**,
isolated **20/48 vs 22/48**. The unwindowed fit-prefix-16 file score
**23/48** is not the frozen slice. Do not sell **24/48** or nested
...
## 2026-09-04 gpt2-medium native rankpath body [4:16) freeze

This freeze names
[PROTOCOL-isolated-rankpath-m12body.md](PROTOCOL-isolated-rankpath-m12body.md)
named before gpt2-medium native rankpath `[4:16)` LRs. Existing `rankpath`
on `experiments/2026-09-01-pair-gpt2-medium-12x4/` with `--model
gpt2-medium --windows 4:16 --fit-prefix 16 --pos-bucket 1`. Do not
leftover-slice. Do not invent those scores. gpt2-medium native opening
stays **6/12**, isolated **22/48 vs 30/48**. Distil native `[4:16)` stays
**9/12**, isolated **25/48 vs 30/48**. Nothing replaces **25/48**.
Isolated-file detection is still not finished. Do not write `thesis/`.
Freeze SHA `37a2c43`.

## 2026-09-04 gpt2-medium native rankpath body [4:16) opened

H-rpm12body **holds**. gpt2-medium native rankpath on `[4:16)` is ranking
**6/12**, isolated **20/48 vs 30/48** (AUC **0.497**; nested **20/48 vs
18/48** at a negative threshold). `used_keys=false`. Native opening
stays **6/12**, isolated **22/48 vs 30/48**. Distil native `[4:16)` stays
**9/12**, isolated **25/48 vs 30/48**. The unwindowed fit-prefix-16 file
score **21/48** is not the frozen slice. File AUC is chance. Do not sell
